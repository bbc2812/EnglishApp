...
async function adaptArticleToLevel(_articleText, level, apiKey, provider, system) {
...
export function registerAiHandlers() {
    ipcMain.handle('ai:chat', async (_event, provider, messages, system, options) => {
        if (provider === 'claude') {
    ...
    ipcMain.handle('ai:adaptArticle', async (_event, articleText, level, options) => {
        const provider = options?.provider ?? 'claude';
        const apiKey = options?.apiKey ?? '';
        if (!apiKey)
            return articleText;
        return adaptArticleToLevel(articleText, level, apiKey, provider, `You are an expert English language curriculum designer. Rewrite the provided text for ${level} level English learners. Use simpler vocabulary, shorter sentences, and more common structures while preserving the core meaning. Return ONLY the rewritten text with no additional commentary.`);
    });
    // --- AI: Summarize content ---
    ipcMain.handle('ai:summarizeContent', async (_event, content, options) => {
        const provider = options?.provider ?? 'claude';
        const apiKey = options?.apiKey ?? '';
        if (!apiKey)
            return 'API key required';
        return adaptArticleToLevel(content, 'B1', apiKey, provider, `Summarize this text in 5-8 key bullet points. Use C1/C2 vocabulary. Keep each point to 2-3 sentences.`);
    });
    ipcMain.handle('ai:isAvailable', async (_event, provider, options) => {
        if (provider === 'ollama') {
            try {
