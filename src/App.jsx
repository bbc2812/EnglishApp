...
function SharedToastContainer() {
    ...
    useEffect(() => {
        ...
        const checkAchievements = async () => {
            const unlockedRows = await window.api.db.all(`SELECT key FROM achievements WHERE unlocked_at IS NOT NULL`);
            ...
            const shadowingBest = await window.api.db.all(`SELECT MAX(match_score) as best FROM pronunciation_sessions`);
            ...
            for (const check of checks) {
             ...
             ON CONFLICT(key) DO NOTHING`, [check.key, check.title, check.title, check.icon]).catch(() => { });
    ...
    return (<>
      <div className="fixed top-4 right-4 z-[9999] space-y-2 max-w-sm">
        <AnimatePresence>
          {toasts.map(toast => (<motion.div key={toast.id} initial={{ opacity: 0, x: 50, scale: 0.9 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: 50, scale: 0.9 }} className="bg-gray-800 border border-gray-700 rounded-lg p-4 shadow-xl flex items-center gap-3">
