...
export function useSRS() {
    const loadDueCards = useCallback(async (limit = 50) => {
        const today = new Date().toISOString().slice(0, 10);
        return window.api.db.all(`SELECT f.*, w.word, w.ipa, w.audio_url, w.definition, w.examples,
    ...
    const loadVocabSetCards = useCallback(async (vocabSetId, limit = 50) => {
        return window.api.db.all(`SELECT f.id, f.word_id, f.due_date, f.stability, f.difficulty,
        ...
        ORDER BY CASE WHEN f.state = 'new' THEN 0 WHEN f.state = 'learning' THEN 1 WHEN f.state = 'review' THEN 2 WHEN f.state = 'relearning' THEN 3 ELSE 4 END,
                 f.due_date ASC
        LIMIT ?`, [vocabSetId, limit]);
    }, []);
    const applyRating = useCallback(async (card, rating) => {
        const result = schedule(card, rating);
        await window.api.db.run(`UPDATE flashcards SET
    ...
    const addWordToFlashcards = useCallback(async (wordId) => {
        await window.api.db.run(`INSERT OR IGNORE INTO flashcards (word_id, due_date, state)
       VALUES (?, date('now'), 'new')`, [wordId]);
    }, []);
    const generateMnemonic = useCallback(async (word) => {
    ...
    const saveMnemonic = useCallback(async (wordId, mnemonic) => {
        await window.api.db.run(`UPDATE words SET mnemonic = ? WHERE id = ?`, [mnemonic, wordId]);
    }, []);
