...
export default function News() {
    ...
    const [guardianItems, setGuardianItems] = useState([]);
    ...
    const fetchNews = useCallback(async (category) => {
        setLoading(true);
        try {
            const data = await window.api.content.fetchNewsAPI(category, newsApiKey);
            setArticles(data);
        }
        catch {
            setArticles([]);
    ...
    const fetchBBCLE = useCallback(async () => {
        setLoading(true);
        try {
            const data = await window.api.content.fetchBBCLE('article');
            setBbcItems(data);
        }
        catch {
            setBbcItems([]);
    ...
    const fetchGuardian = useCallback(async () => {
        setLoading(true);
        try {
            const data = await window.api.content.fetchGuardianArticles();
    ...
    const fetchWOTD = useCallback(async () => {
        const data = await window.api.content.fetchWordOfTheDay();
        setWotd(data);
    }, []);
    const fetchQuotes = useCallback(async () => {
        const data = await window.api.content.fetchQuotable(10);
        setQuotes(data);
    }, []);
    ...
    const handleSave = async (item) => {
        await window.api.db.run(`INSERT INTO saved_articles (url, title, source, content, saved_at)
       VALUES (?, ?, 'News', ?, datetime('now'))
       ON CONFLICT(url) DO UPDATE SET title = ?`, [item.url, item.title, item.description || '', item.title]).catch(() => { });
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex gap-1 mb-6 bg-gray-900 p-1 rounded-lg w-fit">
        ...
        ].map(tab => (<button key={tab.id} onClick={() => loadTab(tab.id)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === tab.id ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white'}`}>
            {tab.label}
          </button>))}
      </div>
      ...
      {/* Tab Content */}
      {loading ? (<div className="card flex items-center justify-center h-48 text-gray-500">Loading…</div>) : activeTab === 'all' && (<div className="space-y-3">
          {articles.length === 0 ? (<div className="card flex items-center justify-center h-48 text-gray-500">
              No articles loaded. Check your NewsAPI key in Settings.
            </div>) : (articles.slice(0, 20).map((article, i) => (<motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                <ArticleCard article={article} onSave={handleSave}/>
              </motion.div>)))}
        </div>)}

      {activeTab === 'bbc' && (<div className="space-y-3">
          {bbcItems.length === 0 ? (<div className="card flex items-center justify-center h-48 text-gray-500">
              Could not load BBC Learning English. Content may be temporarily unavailable.
            </div>) : (bbcItems.map((item, i) => (<motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                <BBCLECard item={item} onSave={handleSave}/>
              </motion.div>)))}
        </div>)}

      {activeTab === 'guardian' && (<div className="space-y-3">
          {guardianItems.length === 0 ? (<div className="card flex items-center justify-center h-48 text-gray-500">
              No Guardian articles found.
            </div>) : (guardianItems.map((item, i) => (<motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                <GuardianCard item={item} onSave={handleSave}/>
              </motion.div>)))}
        </div>)}

      {activeTab === 'wotd' && wotd && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card p-8 text-center">
          ...
          <a href={wotd.url} target="_blank" rel="noopener noreferrer" className="text-sm text-brand-400 hover:text-brand-300">
            🔗 Merriam-Webster Dictionary
          </a>
        </motion.div>)}

      {activeTab === 'quotes' && (<div className="space-y-4">
          {quotes.length === 0 ? (<div className="card flex items-center justify-center h-48 text-gray-500">
              Could not load quotes.
            </div>) : (quotes.map((q, i) => (<motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
...
function ArticleCard({ article, onSave }) {
    ...
    return (<motion.button onClick={() => window.open(article.url, '_blank')} className="card text-left p-5 hover:border-brand-500 transition-colors block w-full">
      <div className="flex items-start justify-between gap-3">
        ...
        <button onClick={(e) => { e.stopPropagation(); onSave(article); setSaved(true); setTimeout(() => setSaved(false), 2000); }} className={`text-xs px-3 py-1.5 rounded flex-shrink-0 ${saved ? 'bg-green-950 text-green-300' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
          {saved ? '✅ Saved' : '💾 Save'}
        </button>
      </div>
    </motion.button>);
}
function BBCLECard({ item, onSave }) {
    const [saved, setSaved] = useState(false);
    return (<div className="card p-5 space-y-3">
      <div className="flex items-center justify-between">
        <CEFRLevel level={item.level || 'B1'}/>
        <button onClick={(e) => { e.stopPropagation(); onSave(item); setSaved(true); setTimeout(() => setSaved(false), 2000); }} className={`text-xs px-3 py-1.5 rounded flex-shrink-0 ${saved ? 'bg-green-950 text-green-300' : 'bg-gray-800 text-gray-400'}`}>
          {saved ? '✅' : '💾'}
        </button>
      </div>
      <h4 className="text-white font-semibold text-sm">{item.title}</h4>
      <p className="text-gray-400 text-xs">{item.description?.substring(0, 150)}...</p>
      {item.audioUrl && (<audio src={item.audioUrl} controls className="w-full mt-2"/>)}
      <button onClick={() => window.open(item.url, '_blank')} className="text-xs text-brand-400 hover:text-brand-300">
        Read full article →
      </button>
    </div>);
}
function GuardianCard({ item, onSave }) {
    const [saved, setSaved] = useState(false);
    return (<div className="card p-5">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs text-gray-500">{item.sectionName}</span>
        <button onClick={(e) => { e.stopPropagation(); onSave({ title: item.webTitle, url: item.webUrl }); setSaved(true); setTimeout(() => setSaved(false), 2000); }} className={`text-xs px-3 py-1.5 rounded flex-shrink-0 ${saved ? 'bg-green-950 text-green-300' : 'bg-gray-800 text-gray-400'}`}>
          {saved ? '✅' : '💾'}
        </button>
      </div>
