...
interface DailyStat {
  ...
  new_words: number
  listening_mins: number
  speaking_mins: number
  writing_mins: number
  xp_earned: number
  streak: number
}
...
export default function Stats(): JSX.Element {
  ...
  const loadData = async () => {
    ...
    const wc = await window.api.db.all(`SELECT COUNT(*) as c FROM words`) as { c: number }[]
    ...
    const ms = await window.api.db.all(
      `SELECT type, count FROM grammar_mistakes ORDER BY count DESC LIMIT 10`
    ) as { type: string; count: number }[]
    setMistakes(ms)

    // Activity breakdown from last 7 days
    const sevenDayStats = await window.api.db.all(
      `SELECT
  ...
  return (
    <div className="p-8 flex flex-col h-full overflow-y-auto">
          ...
          <div className="flex flex-wrap gap-1">
            {heatmapData.map((d, i) => (
              <div
                ...
                style={{ backgroundColor: getHeatColor(d.activity) }}
                title={`${d.date}: ${d.activity} activity`}
              />
            ))}
          </div>
          <div className="flex items-center gap-1 mt-2">
            <span className="text-xs text-gray-500">Less</span>
            {['#1f2937', '#166534', '#15803d', '#22c55e', '#4ade80'].map(c => (
            ...
            return (
              <div key={cefr} className="flex-1">
                ...
                <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${isComplete ? 'bg-brand-500' : 'bg-gray-600'}`}
      ...
      <div className="card">
        ...
        {mistakes.length === 0 ? (
          <p className="text-gray-500 text-sm">No mistakes recorded yet. Keep writing!</p>
        ) : (
