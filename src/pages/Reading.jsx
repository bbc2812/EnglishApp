...
function WordHighlight({ text, onWordClick }) {
    const words = text.split(/(\s+)/);
    return (<span className="selectable">
      {words.map((w, i) => {
            const isWord = /^[a-zA-Z'-]+$/.test(w) && w.length > 2;
            if (!isWord)
                return <span key={i}>{w}</span>;
            return (<span key={i} className="cursor-pointer hover:text-brand-400 hover:bg-brand-950/30 px-0.5 rounded transition-colors" onClick={() => onWordClick(w)}>
            {w}
          </span>);
        })}
    </span>);
...
function MCQExercise({ exercise, onAnswer }) {
    ...
    return (<div className="space-y-3">
      ...
      <div className="space-y-2">
        {options.map((opt, i) => (<button key={i} onClick={() => !submitted && setSelected(opt)} disabled={submitted} className={`w-full text-left px-4 py-2.5 rounded-lg border text-sm transition-all ${submitted
      ...
      {!submitted && (<button onClick={handleSubmit} disabled={!selected} className="btn-primary px-6 py-2 text-sm disabled:opacity-40">
          Check Answer
        </button>)}
      {submitted && (<p className={`text-sm font-medium ${isCorrect ? 'text-green-400' : 'text-red-400'}`}>
...
function FillExercise({ exercise, onAnswer }) {
    ...
    return (<div className="space-y-3">
      ...
      <div className="flex gap-2">
        <input type="text" value={input} onChange={e => setInput(e.target.value)} disabled={submitted} className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:border-brand-500 focus:outline-none disabled:opacity-50" placeholder="Type your answer..." onKeyDown={e => e.key === 'Enter' && !submitted && handleSubmit()}/>
        {!submitted ? (<button onClick={handleSubmit} className="btn-primary px-5 py-2.5 text-sm" disabled={!input.trim()}>
            Check
          </button>) : (<div className={`px-4 py-2.5 rounded-lg text-sm font-medium ${isCorrect ? 'bg-green-950/50 text-green-300' : 'bg-red-950/50 text-red-300'}`}>
            {isCorrect ? '✅ Correct!' : `Answer: ${exercise.answer}`}
          </div>)}
      </div>
    </div>);
}
function ExerciseCard({ exercise, onAnswer, index }) {
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="card p-5">
      ...
      {exercise.type === 'mcq' ? (<MCQExercise exercise={exercise} onAnswer={onAnswer}/>) : (<FillExercise exercise={exercise} onAnswer={onAnswer}/>)}
    </motion.div>);
}
export default function Reading() {
    ...
    const loadLesson = useCallback(async () => {
        if (!lessonId)
            return;
        setLoading(true);
        const les = await window.api.db.all(`SELECT * FROM lessons WHERE id = ?`, [lessonId]);
        if (les.length === 0) {
            setLoading(false);
            return;
        }
        setLesson(les[0]);
        const exs = await window.api.db.all(`SELECT * FROM exercises WHERE lesson_id = ? ORDER BY id`, [lessonId]);
        setExercises(exs);
    ...
    const handleExerciseAnswer = (index, correct) => {
        setAnswers(prev => {
            const next = [...prev];
            next[index] = correct;
            return next;
    ...
    const handleComplete = async () => {
        ...
        try {
            await window.api.db.run(`INSERT INTO daily_stats (date, xp_earned)
         VALUES (?, ?)
         ON CONFLICT(date) DO UPDATE SET xp_earned = xp_earned + ?`, [today, xpEarned, xpEarned]);
        }
        catch { /* ignore */ }
        await loadUnits();
        if (score >= 80) {
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <AnimatePresence mode="wait">
        {phase === 'intro' && (<motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            <div className="card p-6 text-center">
              ...
              <div className="flex gap-3 justify-center mt-4">
                <button onClick={() => { setPhase('quickread'); setQuickReadIndex(0); }} className="btn-secondary px-6 py-2.5 text-sm">
                  ⚡ Quick Read Summary
                </button>
                <button onClick={() => setPhase('reading')} className="btn-primary px-8 py-3 text-base">
                  Start Reading
                </button>
              </div>
            </div>
        ...
        {phase === 'quickread' && (<motion.div key="quickread" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="card p-8 text-center min-h-[280px] flex flex-col items-center justify-center">
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-4">
                {MOCK_QUICK_READ[quickReadIndex].vocab.map((v, i) => (<span key={i} className="text-xs bg-brand-950/30 text-brand-300 px-2 py-0.5 rounded">
                    {v}
                  </span>))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex gap-2">
                {MOCK_QUICK_READ.map((_, i) => (<div key={i} className={`h-1.5 rounded-full transition-all ${i === quickReadIndex ? 'w-6 bg-brand-400' : 'w-1.5 bg-gray-700'}`}/>))}
              </div>
              <div className="flex gap-2">
                <button onClick={() => setQuickReadIndex(prev => Math.max(0, prev - 1))} disabled={quickReadIndex === 0} className="btn-secondary px-4 py-2 text-sm disabled:opacity-40">
                  ← Prev
                </button>
                <button onClick={() => setQuickReadIndex(prev => Math.min(MOCK_QUICK_READ.length - 1, prev + 1))} disabled={quickReadIndex === MOCK_QUICK_READ.length - 1} className="btn-secondary px-4 py-2 text-sm disabled:opacity-40">
                  Next →
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => setPhase('intro')} className="btn-secondary px-6 py-2.5 text-sm">
                ← Back
              </button>
              <button onClick={() => { setPhase('reading'); setQuickReadIndex(0); }} className="btn-primary px-6 py-2.5 text-sm">
                Read Full Article →
              </button>
            </div>
          </motion.div>)}

        {phase === 'reading' && (<motion.div key="reading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Article */}
            <div className="card p-6 selectable">
              ...
              <div className="space-y-3 text-gray-300 leading-relaxed text-base">
                {lesson.transcript.split('. ').filter(Boolean).map((sentence, i) => (<p key={i} className="leading-relaxed">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-gray-800 rounded-full">
                <div className="h-full bg-brand-500 rounded-full transition-all" style={{ width: `${(answers.filter(a => a !== null).length / exercises.length) * 100}%` }}/>
              </div>
            ...
            <div className="space-y-4">
              ...
              {exercises.map((ex, i) => (<ExerciseCard key={ex.id} exercise={ex} onAnswer={(correct) => handleExerciseAnswer(i, correct)} index={i}/>))}
            </div>

            {allAnswered && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <div className="card p-5 text-center">
                  ...
                  {score >= 80 ? (<p className="text-green-400 text-sm mb-3">🎉 Great job! You passed! Click below to save your progress.</p>) : (<p className="text-amber-400 text-sm mb-3">Keep practicing! You need 80% to unlock the next unit.</p>)}
                  <div className="flex gap-3 justify-center">
                    <button onClick={() => { setPhase('intro'); setAnswers(new Array(exercises.length).fill(null)); }} className="btn-secondary px-6 py-2.5 text-sm">
                      Retry
                    </button>
                    <button onClick={handleComplete} className="btn-primary px-6 py-2.5 text-sm">
                      Save & Continue
                    </button>
                  </div>
                </div>
        ...
        {phase === 'results' && completedAt > 0 && (<motion.div key="results" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card p-8 text-center">
            ...
            {score >= 80 && (<p className="text-green-400 text-sm mb-4">📈 Unit progress updated!</p>)}
            <div className="flex gap-3 justify-center">
              <button onClick={() => { setPhase('intro'); setAnswers(new Array(exercises.length).fill(null)); setCompletedAt(0); }} className="btn-secondary px-6 py-2.5 text-sm">
                Try Again
              </button>
