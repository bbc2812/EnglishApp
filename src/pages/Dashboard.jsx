...
function CEFRBadge({ level }) {
    ...
    return (<span className={`text-xs font-bold px-1.5 py-0.5 rounded ${colorMap[level] || 'bg-gray-600 text-gray-200'}`}>
      {level}
    </span>);
}
function CompletionRing({ percent, size = 40, strokeWidth = 3, unlocked }) {
    ...
    return (<div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        ...
        {percent > 0 && (<circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className="transition-all duration-500"/>)}
      </svg>
      {!unlocked && (<span className="absolute text-gray-500 text-xs" title="Locked">
          🔒
        </span>)}
    </div>);
}
...
function generatePath(positions) {
...
function RoadmapSVG({ units, onUnitClick, }) {
    ...
    return (<div className="card overflow-x-auto pb-2">
      <div className="min-w-[700px]">
        <svg width="1480" height="200" className="overflow-visible">
          {/* Background path */}
          <path d={pathD} fill="none" stroke="#1f2937" strokeWidth="4" strokeLinecap="round"/>
          {/* Animated progress path */}
          <motion.path d={pathD} fill="none" stroke="url(#pathGradient)" strokeWidth="4" strokeLinecap="round" initial={{ strokeDasharray: pathLength, strokeDashoffset: pathLength }} animate={{ strokeDashoffset: pathLength - (unlockedCount / totalUnits) * pathLength }} transition={{ duration: 1.5, ease: 'easeInOut' }}/>
          {/* Gradient definition */}
          <defs>
          ...
          {units.map((unit, i) => {
            ...
            return (<motion.g key={unit.id} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.08, type: 'spring', stiffness: 200, damping: 12 }} className="cursor-pointer" onClick={() => onUnitClick(unit)}>
                {/* Glow effect for unlocked units */}
                {isUnlocked && (<motion.circle cx={pos.x} cy={pos.y + 20} r="28" fill="none" stroke={isComplete ? '#22c55e' : '#06b6d4'} strokeWidth="1" opacity="0.2" animate={{ r: [26, 32, 26], opacity: [0.1, 0.3, 0.1] }} transition={{ duration: 3, repeat: Infinity, repeatType: 'reverse' }}/>)}
                {/* Completion ring */}
                <CompletionRing percent={unit.percent_complete} size={52} strokeWidth={3} unlocked={unit.unlocked}/>
                {/* Unit number badge */}
                <circle cx={pos.x} cy={pos.y + 20} r={isUnlocked ? 22 : 20} fill={isUnlocked ? '#111827' : '#1f2937'} stroke={isComplete ? '#22c55e' : isUnlocked ? '#374151' : '#4b5563'} strokeWidth={isComplete ? 2 : 1}/>
                {/* Unit title */}
                <text x={pos.x} y={pos.y + 62} textAnchor="middle" fill={isUnlocked ? '#e5e7eb' : '#6b7280'} fontSize="11" fontWeight={isUnlocked ? 600 : 400} className="select-none pointer-events-none">
                  {unit.title}
                </text>
                {/* CEFR level */}
                <text x={pos.x} y={pos.y + 76} textAnchor="middle" fill="#9ca3af" fontSize="9" className="select-none pointer-events-none">
                  {unit.cefr_level}
                </text>
                {/* Percent text inside ring */}
                {isUnlocked && unit.percent_complete > 0 && (<text x={pos.x} cy={pos.y + 20} textAnchor="middle" dominantBaseline="central" fill={isComplete ? '#22c55e' : '#e5e7eb'} fontSize="9" fontWeight="bold" className="select-none pointer-events-none">
...
function StatCard({ icon, label, value, subValue }) {
    return (<motion.div whileHover={{ y: -2 }} className="card text-center">
...
function VocabSetsSection({ vocabSets, onSetClick, }) {
    if (vocabSets.length === 0)
        return null;
    return (<div>
      <h3 className="text-lg font-semibold text-white mb-3">Topical Vocabulary Sets</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        {vocabSets.map((set, i) => (<motion.button key={set.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} onClick={() => onSetClick(set)} className="card text-left p-3 hover:border-brand-500 transition-colors">
...
export default function Dashboard() {
    ...
    const loadDashboard = useCallback(async () => {
        setLoading(true);
        await loadUnits();
        const [dailyStats, vocabSetsData, lessonCount, wordCount, unlockedLessons, challenge, achievements, streakData] = await Promise.all([
            window.api.db.all(`SELECT * FROM daily_stats WHERE date = ? ORDER BY date DESC LIMIT 1`, [new Date().toISOString().slice(0, 10)]),
            window.api.db.all(`SELECT id, title, topic, level FROM vocab_sets ORDER BY id`),
            window.api.db.all(`SELECT COUNT(*) as count FROM lesson_progress WHERE completed_at LIKE ?`, [`${new Date().toISOString().slice(0, 10)}%`]),
            window.api.db.all(`SELECT COUNT(*) as count FROM words`),
            window.api.db.all(`SELECT id, title, type, locked FROM lessons WHERE unit_id IN (SELECT id FROM units WHERE unlocked = 1) AND type IN ('listening', 'reading') ORDER BY unit_id, id`),
            window.api.db.all(`SELECT type, completed, xp_reward FROM daily_challenges WHERE date = date('now') ORDER BY date LIMIT 1`),
            window.api.db.all(`SELECT COUNT(*) as count FROM achievements WHERE unlocked_at IS NOT NULL`),
            window.api.db.all(`SELECT COUNT(*) as streak FROM (
        ...
        const lessonsResult = lessonCount;
        const wordsResult = wordCount;
        const lessonC = Array.isArray(lessonsResult) ? lessonsResult[0]?.count || 0 : lessonsResult?.count || 0;
        const wordC = Array.isArray(wordsResult) ? wordsResult[0]?.count || 0 : wordsResult?.count || 0;
        ...
        if (challengeStreak >= 7 && !achievements) {
            window.api.db.run(`INSERT INTO achievements (key, title, description, icon, unlocked_at)
         VALUES ('daily_warrior', '⚔️ Daily Warrior', 'Complete 7 daily challenges', '⚔️', datetime('now'))
         ON CONFLICT(key) DO NOTHING`).then(() => { }).catch(() => { });
            // Add streak bonus XP
            window.api.db.run(`INSERT INTO daily_stats (date, xp_earned, streak)
         VALUES (?, 100, (SELECT COALESCE(streak, 0) FROM daily_stats WHERE date = ?))
         ON CONFLICT(date) DO UPDATE SET xp_earned = xp_earned + 100`, [new Date().toISOString().slice(0, 10), new Date().toISOString().slice(0, 10)]).then(() => { }).catch(() => { });
        }
        setLoading(false);
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      {/* Daily Challenge */}
      {dailyChallenge && (<div className="card p-4 mb-6 border-brand-900/50 bg-brand-950/10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs">
                  {dailyChallenge.completed ? (<span className="text-green-400">✅ Completed! +{dailyChallenge.xp_reward} XP</span>) : (<span>
            ...
            {!dailyChallenge.completed && (<button onClick={() => navigate('/flashcards')} className="btn-primary px-4 py-2 text-xs">
                Start
              </button>)}
          </div>
        </div>)}
      ...
      {/* Achievements */}
      {achievementCount > 0 && (<div className="flex items-center gap-2 mb-6">
          <span className="text-sm text-gray-500">🏅 Achievements:</span>
          <div className="flex gap-1 flex-wrap">
            {achievementCount > 0 && (<span className="text-xs bg-gray-800 text-gray-300 px-2 py-0.5 rounded">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <motion.button onClick={() => navigate('/listening')} whileHover={{ y: -2 }} className="card p-4 text-left">
          <p className="text-2xl mb-2">🎧</p>
        ...
        <motion.button onClick={() => navigate('/tutor')} whileHover={{ y: -2 }} className="card p-4 text-left">
          <p className="text-2xl mb-2">🤖</p>
      ...
      {lessons.length > 0 && (<div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {lessons.map((lesson, i) => (<motion.button key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} onClick={() => navigate(`/${lesson.type}?lesson=${lesson.id}`)} disabled={lesson.locked === 1} className="card text-left p-4 hover:border-brand-500 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">{lesson.type === 'listening' ? '🎧' : '📖'}</span>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded ${lesson.type === 'listening' ? 'bg-blue-950 text-blue-300' : 'bg-green-950 text-green-300'}`}>
