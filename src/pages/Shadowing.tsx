...
type Phase = 'drill' | 'play' | 'record' | 'score'
...
function WaveformBox({ id }: { id: string; color?: string }): JSX.Element {
...
function ProblemSounds(): JSX.Element {
  ...
  return (
    <div className="card p-5">
      ...
      <div className="space-y-2">
        {problems.map((p, i) => (
          <div key={i} className="flex items-start gap-3 p-2 rounded bg-gray-800/50">
  ...
  const startRecording = useCallback(() => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
      ...
      mediaRecorder.onstop = async () => {
        ...
        let fb = ''
        if (simulatedScore >= 90) fb = 'Excellent! Your pronunciation is very close to native.'
        else if (simulatedScore >= 70) fb = 'Good effort! Focus on the /θ/ sound — press your tongue between your teeth.'
        else if (simulatedScore >= 50) fb = 'Fair. Try listening to the native audio 3 more times before recording.'
        else fb = `Keep practicing! Your common mistake: ${current.commonMistake}`
        setFeedback(fb)
        setPhase('score')
        ...
        try {
          await window.api.db.run(
            `INSERT INTO pronunciation_sessions (word, match_score, phoneme_feedback, created_at)
             VALUES (?, ?, ?, datetime('now'))`,
            [current.word, simulatedScore, fb]
          )
        } catch { /* ignore */ }
  ...
  return (
    <div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <AnimatePresence mode="wait">
        {phase === 'drill' && (
          <motion.div key="drill" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Drill selector */}
            <div className="grid grid-cols-3 gap-3">
              {DRILLS.map((d, i) => (
                <button
                  key={i}
                  onClick={() => { setDrillIndex(i); resetPhase() }}
                  className={`card p-3 text-left ${i === drillIndex ? 'border-brand-500 bg-brand-950/30' : 'hover:border-gray-600'}`}
                >
            ...
            <div className="card p-6 text-center">
              ...
              <div className="mt-6 space-y-2">
                ...
                {current.sentences.map((s, i) => (
                  <div key={i} className="text-left px-4 py-2.5 rounded-lg bg-gray-800/50 text-gray-300 text-sm">
                    "{s}"
                  </div>
                ))}
              </div>
        ...
        {phase === 'record' && (
          <motion.div key="record" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            <div className="card p-6 text-center">
              ...
              {/* Timer */}
              {recording && (
                <p className="text-red-400 text-lg font-mono mb-4">⏱ {recordingTime}s</p>
              )}
              ...
              {!recording ? (
                <button onClick={startRecording} className="btn-primary px-10 py-3 text-lg bg-red-600 hover:bg-red-500">
                  🎤 Record Now
                </button>
              ) : (
                <button onClick={stopRecording} className="btn-secondary px-10 py-3 text-lg bg-gray-700">
                  ⏹ Stop
                </button>
              )}
            </div>
        ...
        {phase === 'score' && (
          <motion.div key="score" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
            <div className="card p-6 text-center">
