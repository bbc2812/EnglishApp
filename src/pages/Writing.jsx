...
function GrammarCorrection({ original, correction, explanation }) {
    return (<div className="p-4 bg-gray-800/50 rounded-lg space-y-2">
...
export default function Writing() {
    ...
    const loadMistakes = useCallback(async () => {
        const rows = await window.api.db.all(`SELECT * FROM grammar_mistakes ORDER BY count DESC LIMIT 20`);
        setMistakes(rows);
    }, []);
    const loadHistory = useCallback(async () => {
        const rows = await window.api.db.all(`SELECT * FROM writing_history ORDER BY created_at DESC LIMIT 20`);
        setHistory(rows);
    }, []);
    useEffect(() => {
        loadMistakes();
    ...
    const simulateAnalysis = () => {
        ...
        setSubmitted(true);
        // Save to DB
        window.api.db.run(`INSERT INTO writing_history (prompt, content, score, created_at)
       VALUES (?, ?, ?, datetime('now'))`, [prompt, essay, fakeScore]).then(() => {
            const xpEarned = fakeScore >= 80 ? 15 : 10;
            setTodayXP(xpEarned);
            // Update daily stats with XP
            const today = new Date().toISOString().slice(0, 10);
            window.api.db.run(`INSERT INTO daily_stats (date, xp_earned, writing_mins)
         VALUES (?, ?, 1)
         ON CONFLICT(date) DO UPDATE SET
           xp_earned = xp_earned + ?,
           writing_mins = writing_mins + 1`, [today, xpEarned, xpEarned]).catch(() => { });
        }).catch(() => { });
        ...
        generatedCorrections.forEach(c => {
            ...
            window.api.db.run(`INSERT INTO grammar_mistakes (type, description, count, last_seen)
         VALUES (?, ?, COALESCE((SELECT count FROM grammar_mistakes WHERE type = ?), 0) + 1, datetime('now'))
         ON CONFLICT(type) DO UPDATE SET count = count + 1, last_seen = datetime('now')`, [mistakeType, c.explanation]).catch(() => { });
        });
        loadMistakes();
    };
    const formatTime = (s) => {
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex gap-1 mb-6 bg-gray-900 p-1 rounded-lg w-fit">
        {['write', 'mistakes', 'history'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === tab ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white'}`}>
            {tab === 'write' ? '✍️ Write' : tab === 'mistakes' ? '⚠️ Top Mistakes' : '📜 History'}
          </button>))}
      </div>

      <AnimatePresence mode="wait">
        {activeTab === 'write' && (<motion.div key="write" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Prompt */}
            <div className="card p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {running ? (<span className="text-xs text-green-400 animate-pulse">● Writing...</span>) : submitted ? (<span className="text-xs text-amber-400">✓ Complete</span>) : (<span className="text-xs text-gray-500">10 min</span>)}
                </div>
              </div>
            ...
            {/* Essay textarea */}
            <div className="card p-5">
              <textarea value={essay} onChange={e => setEssay(e.target.value)} disabled={submitted} placeholder="Write your essay here... (minimum 20 words)" className="w-full h-64 bg-gray-800 border border-gray-700 rounded-lg p-4 text-white resize-none focus:border-brand-500 focus:outline-none disabled:opacity-50 leading-relaxed"/>
              <div className="flex items-center justify-between mt-3">
                <span className="text-xs text-gray-500">{essay.split(/\s+/).filter(Boolean).length} words</span>
                <div className="flex gap-2">
                  {!submitted ? (<>
                      <button onClick={() => setRunning(!running)} className="btn-secondary px-4 py-2 text-sm">
                        {running ? '⏸ Pause' : '▶ Start Timer'}
                      </button>
                      <button onClick={handleSubmit} disabled={essay.trim().length < 20} className="btn-primary px-6 py-2 text-sm disabled:opacity-40">
                        Submit for Review
                      </button>
                    </>) : (<button onClick={() => {
                    setSubmitted(false);
            ...
            {/* Results */}
            {submitted && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                {/* Score */}
                ...
                {/* Corrections */}
                {corrections.length > 0 && (<div className="space-y-3">
                    <h4 className="text-sm font-semibold text-white">Grammar Corrections</h4>
                    {corrections.map((c, i) => (<GrammarCorrection key={i} {...c}/>))}
                  </div>)}
              </motion.div>)}
          </motion.div>)}

        {activeTab === 'mistakes' && (<motion.div key="mistakes" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <h3 className="text-lg font-semibold text-white">Top 20 Vietnamese→English Errors</h3>
            {TOP_MISTAKES.map(m => (<div key={m.id} className="card p-4">
                <div className="flex items-start justify-between mb-2">
                  <h4 className="text-sm font-semibold text-white">{m.id}. {m.type}</h4>
                  {mistakes.find(mk => mk.type === m.type) && (<span className="text-xs bg-red-950 text-red-300 px-2 py-0.5 rounded">
        ...
        {activeTab === 'history' && (<motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            <h3 className="text-lg font-semibold text-white">Writing History</h3>
            {history.length === 0 ? (<div className="card flex items-center justify-center h-48 text-gray-500">No writing history yet</div>) : (history.map((h, i) => (<div key={i} className="card p-4">
