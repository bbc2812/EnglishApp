...
function DifficultyBadge({ level }) {
    ...
    return (<span className={`text-xs font-bold px-1.5 py-0.5 rounded ${colors[level] || 'bg-gray-700 text-gray-300'}`}>
      {level}
    </span>);
}
function SpeakingPromptCard({ prompt, onSelect }) {
    const categoryIcons = {
        debate: '🗣️',
        opinion: '💭',
        description: '📝',
    ...
    return (<motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={() => onSelect(prompt)} className="card text-left p-5 hover:border-brand-500 transition-colors block w-full">
      ...
      <div className="flex flex-wrap gap-1 mt-2">
        {prompt.vocabHints.slice(0, 3).map((v, i) => (<span key={i} className="text-xs text-brand-400 bg-brand-950/30 px-1.5 py-0.5 rounded">
            {v}
          </span>))}
      </div>
    </motion.button>);
...
function SpeakingTimer({ running, timeLeft, onToggle }) {
    ...
    return (<div className="flex items-center gap-3">
      ...
      <button onClick={onToggle} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${running ? 'bg-amber-600 hover:bg-amber-500 text-white' : 'bg-green-600 hover:bg-green-500 text-white'}`}>
        {running ? '⏸ Pause' : '▶ Start'}
      </button>
    </div>);
}
function SpeakingFeedback({ score, tips }) {
    return (<div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {tips.map((tip, i) => (<div key={i} className="flex items-start gap-2 text-sm text-gray-300">
...
export default function Speaking() {
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <AnimatePresence mode="wait">
        {phase === 'prompt' && (<motion.div key="prompt" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {SPEAKING_PROMPTS.map((p) => (<SpeakingPromptCard key={p.id} prompt={p} onSelect={handleSelectPrompt}/>))}
            </div>
          </motion.div>)}

        {phase === 'speaking' && selectedPrompt && (<motion.div key="speaking" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Prompt Card */}
            <div className="card p-6 border-brand-900/50">
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedPrompt.vocabHints.map((v, idx) => (<span key={idx} className="text-sm bg-brand-950/30 text-brand-300 px-2 py-0.5 rounded">
                      {v}
                    </span>))}
                </div>
              </div>
              ...
              {/* Transcript preview */}
              {transcript && (<div className="mt-4 p-3 bg-gray-800/50 rounded-lg">
            ...
            <div className="card p-5 text-center">
              ...
              {!running && (<button onClick={handleStartSpeaking} className="btn-primary px-8 py-3 text-base">
                  ▶ Start Speaking
                </button>)}
              {running && timeLeft > 0 && (<button onClick={handleEndSpeaking} className="btn-secondary px-8 py-3 text-base">
                  ⏹ Stop & Review
                </button>)}
            </div>
          </motion.div>)}

        {phase === 'review' && (<motion.div key="review" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
            <div className="card p-8">
              ...
              {selectedPrompt && (<div className="mt-6 pt-6 border-t border-gray-800">
