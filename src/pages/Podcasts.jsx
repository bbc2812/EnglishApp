...
function ClickableWord({ word, onWordClick }) {
    ...
    return (<span onClick={(e) => { e.stopPropagation(); onWordClick(word); }} className="cursor-pointer hover:text-brand-400 hover:bg-brand-950/30 px-0.5 rounded transition-colors inline">
      {word}
    </span>);
}
function SubtitleLine({ sentence, translation, isActive, onClick, onWordClick }) {
    return (<div onClick={onClick} className={`px-3 py-2 rounded-lg cursor-pointer transition-all ${isActive
      ...
      <p className={`text-sm ${isActive ? 'text-white font-medium' : 'text-gray-300'} selectable`}>
        {sentence.split(' ').map((w, i) => (<ClickableWord key={i} word={w} onWordClick={onWordClick}/>))}
      </p>
...
function AudioPlayer({ episode, onWordClick }) {
    const [playing, setPlaying] = useState(false);
    const [progress, setProgress] = useState(0);
    const [speed, setSpeed] = useState(1);
    const [currentTime, setCurrentTime] = useState(0);
    const [subtitleMode, setSubtitleMode] = useState('en-vn');
    const [loopSentence, setLoopSentence] = useState(null);
    ...
    useEffect(() => {
        ...
        soundRef.current = new Howl({
            ...
            onpause: () => { setPlaying(false); if (intervalRef.current)
                clearInterval(intervalRef.current); }
        });
        return () => { if (intervalRef.current)
            clearInterval(intervalRef.current); };
    }, [episode.audioUrl]);
    const activeTranscriptIndex = episode.transcript.findIndex(t => currentTime >= t.startTime && currentTime < t.endTime);
    return (<div className="card p-4 space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          {[0.5, 0.75, 1, 1.25, 1.5, 2].map(s => (<button key={s} onClick={() => changeSpeed(s)} className={`text-xs px-2 py-0.5 rounded ${speed === s ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              {s}x
            </button>))}
        </div>
        <div className="flex items-center gap-1 border-l border-gray-700 pl-3">
          <span className="text-xs text-gray-500">Subs:</span>
          {['en', 'en-vn', 'vn'].map(m => (<button key={m} onClick={() => setSubtitleMode(m)} className={`text-xs px-2 py-0.5 rounded ${subtitleMode === m ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              {m}
            </button>))}
        </div>
      </div>
      ...
      <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
        {episode.transcript.map((t, i) => {
            ...
            if (subtitleMode === 'en' && !isActive)
                return null;
            if (subtitleMode === 'vn' && isActive)
                return null;
            return (<div key={i} onClick={() => playSentence(i)} className={`px-2 py-1 rounded cursor-pointer text-sm transition-all ${isActive ? 'bg-brand-950/40 text-white font-medium' : 'text-gray-500 hover:text-gray-300'}`}>
...
export default function Podcasts() {
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!selectedEpisode && (<button onClick={() => navigate('/')} className="btn-secondary px-4 py-2 text-sm">← Back</button>)}
      </div>

      <AnimatePresence mode="wait">
        {!selectedEpisode ? (<motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
            {loading ? (<div className="card flex items-center justify-center h-48 text-gray-500">Loading episodes…</div>) : (episodeList.map((ep, i) => (<motion.button key={ep.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} onClick={() => setSelectedEpisode(ep)} className="card w-full text-left p-5 hover:border-brand-500 transition-colors">
          ...
          </motion.div>) : (<motion.div key="player" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Player */}
            ...
            <div className="card p-5">
              ...
              <div className="space-y-2">
                {selectedEpisode.transcript.map((t, i) => (<SubtitleLine key={i} sentence={t.sentence} translation={t.translation} isActive={false} onClick={() => { }} onWordClick={handleWordClick}/>))}
              </div>
            </div>
