...
function AudioPlayer({ url, transcript }) {
        ...
        soundRef.current = new Howl({
            ...
            onplay: () => {
                ...
                intervalRef.current = setInterval(() => {
                    if (soundRef.current) {
                        const curr = soundRef.current.seek();
                        const dur = soundRef.current.duration();
                        setCurrentTime(curr);
                        setDuration(dur);
                        setProgress((curr / dur) * 100);
                        if (transcript) {
                            const sentenceIndex = Math.min(Math.floor((curr / dur) * 3), 2);
    ...
    if (!url || !transcript) {
        return (<div className="card p-4 bg-gray-800/50">
    ...
    return (<div className="card p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        {/* Progress bar */}
        <div className="flex-1 h-2 bg-gray-700 rounded-full cursor-pointer relative" onClick={seekTo}>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {[0.5, 0.75, 1, 1.25, 1.5, 2].map(s => (<button key={s} onClick={() => changeSpeed(s)} className={`text-xs px-2 py-0.5 rounded ${speed === s ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
            {s}x
          </button>))}
      </div>
      ...
      <div className="space-y-2 mt-3">
        {sentences.map((sentence, i) => {
            ...
            return (<p key={i} className={`text-sm leading-relaxed transition-all ${isHighlighted
...
export default function Listening() {
    ...
    const handleComplete = async () => {
        ...
        try {
            await window.api.db.run(`INSERT INTO daily_stats (date, xp_earned, listening_mins)
         VALUES (?, ?, 5)
         ON CONFLICT(date) DO UPDATE SET
           xp_earned = xp_earned + ?,
           listening_mins = listening_mins + 5`, [today, xpEarned, xpEarned]);
        }
        catch { /* ignore */ }
        await loadUnits();
        if (score >= 80) {
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'intro' && (<motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            <div className="card p-6 text-center">
              ...
              <button onClick={() => setPhase('listening')} className="btn-primary px-8 py-3 text-base mt-4">
                Start Listening
              </button>
            </div>

            {/* Transcript preview */}
            {lesson.transcript && (<div className="card p-5">
        ...
        {phase === 'listening' && (<motion.div key="listening" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
            {/* Audio player */}
            ...
            {allAnswered && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <div className="card p-5 text-center">
                  ...
                  {score >= 80 ? (<p className="text-green-400 text-sm mb-3">🎉 Great job! You passed! Click below to save your progress.</p>) : (<p className="text-amber-400 text-sm mb-3">Keep practicing! You need 80% to unlock the next unit. Review the transcript and try again.</p>)}
        ...
        {phase === 'results' && completedAt > 0 && (<motion.div key="results" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card p-8 text-center">
            ...
            {score >= 80 && (<p className="text-green-400 text-sm mb-4">
