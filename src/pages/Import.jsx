...
export default function Import() {
    ...
    const [activeLevel, setActiveLevel] = useState('original');
    ...
    const handleImport = useCallback(async () => {
        ...
        setImporting(true);
        try {
            const data = await window.api.content.scrapeUrl(url);
            setArticle({ ...data, url });
            ...
            try {
                ...
                setSummary('AI summary unavailable — set your API key in Settings.');
            }
    ...
    const handleReadingComplete = useCallback(() => {
        ...
        // Save to DB
        if (article) {
            window.api.db.run(`INSERT INTO saved_articles (url, title, source, content, summary, reading_mins, comprehension_score, saved_at)
         VALUES (?, ?, 'Imported', ?, ?, ?, ?, datetime('now'))
         ON CONFLICT(url) DO UPDATE SET
           title = ?, content = ?, summary = ?, reading_mins = ?, comprehension_score = ?`, [url, article.title, article.content, summary, Math.max(elapsed, 0.1), comprehensionScore,
                article.title, article.content, summary, Math.max(elapsed, 0.1), comprehensionScore]).catch(() => { });
        }
    ...
    useEffect(() => {
        if (!startReading || !currentContent)
            return;
    ...
    return (<div className="p-8 flex flex-col h-full overflow-y-auto">
      ...
      <div className="card p-5 mt-6">
        <div className="flex gap-3">
          <input type="url" value={url} onChange={e => setUrl(e.target.value)} placeholder="https://example.com/article..." className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:border-brand-500 focus:outline-none"/>
          <button onClick={handleImport} disabled={importing || !url.trim()} className="btn-primary px-6 py-3 disabled:opacity-40">
            {importing ? '⏳ Importing…' : '📥 Import'}
          </button>
        </div>
      </div>

      {article && !startReading && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 space-y-6">
          {/* Article info */}
          <div className="card p-5">
            ...
            <div className="flex gap-2 mb-4">
              {['original', 'B1', 'B2', 'C1'].map(level => (<button key={level} onClick={() => setActiveLevel(level)} className={`px-3 py-1.5 rounded text-xs font-medium ${activeLevel === level ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
                  {level === 'original' ? 'Original' : level}
                </button>))}
            </div>
            {summary && (<div className="p-3 bg-gray-800/50 rounded-lg mb-4">
      ...
      {startReading && currentContent && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-6 space-y-4">
          <div className="card p-5">
            ...
            <div className="mt-4 flex gap-3 justify-end">
              <button onClick={handleReadingComplete} className="btn-primary px-6 py-2.5 text-sm">
                ✅ Done Reading
              </button>
            </div>
          </div>
