...
function XPBar({ xp, level }) {
    ...
    return (<div className="px-2 py-3 border-t border-gray-800">
      <div className="space-y-1">
        ...
        <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-brand-600 to-brand-400 rounded-full transition-all" style={{ width: `${progress}%` }}/>
        </div>
      </div>
    </div>);
}
export default function Sidebar() {
    ...
    useEffect(() => {
        const loadAllXP = async () => {
            let total = 0;
            try {
                const writing = await window.api.db.all(`SELECT SUM(score || 0) as total_xp FROM writing_history`);
            ...
            try {
                ...
                const dailyStats = await window.api.db.all(`SELECT xp_earned FROM daily_stats WHERE date = ?`, [today]);
                total += (dailyStats[0]?.xp_earned || 0);
            }
            catch { /* ignore */ }
            try {
                const userXp = await window.api.db.all(`SELECT total_xp FROM user_xp ORDER BY updated_at DESC LIMIT 1`);
    ...
    return (<aside className="w-52 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col h-full">
      {/* Logo */}
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {NAV.map(({ to, icon, label }) => (<NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `sidebar-link ${isActive ? 'active' : ''}`}>
